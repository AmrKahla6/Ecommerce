...
<div class="form-group form-md-line-input  @if($errors->has(''.$slot)) has-error @endif" @isset($block_id) id="{{ $block_id }}" @endisset>
    ...
    <div class="col-md-10">
        <input type="{{ $type }}" class="form-control" @if(!isset($id)) id="{{ $slot }}" @endif name="{{ $slot }}" placeholder="{{ __('Enter the') }} {{ __(''.$label) }}" @if(isset($value)) value="{{ $value }}"  @else value="{{ old(''.$slot) }}" @endif>
