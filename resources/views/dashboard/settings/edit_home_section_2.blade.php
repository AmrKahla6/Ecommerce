...
@endsection
@section('content')
<form role="form" class="form-horizontal" method="POST" action="{{route('save_edit_home_section_2',$homesection2->id)}}"  enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="form-body">
        @component('input_trans', ['type' => 'text', 'label' => 'title', 'required' => false, 'model' => $homesection2])
            title
        @endcomponent
        @component('input_trans', ['type' => 'text', 'label' => 'description', 'required' => false, 'model' => $homesection2])
            description
        @endcomponent
        ...
        <div class="form-group form-md-line-input">
            ...
            <div class="col-md-10">
                <select class="js-example-basic-single js-states form-control" id="subcategory" name="subcategory">
                    ...
                    <option value="{{ $subcategory->id }}" {{ old('subcategory') == $subcategory->id || $homesection2->subcategory_id== $subcategory->id  ? 'selected' : '' }} >{{ $subcategory->name }}</option>
                    @endforeach
                </select>
                <div class="form-control-focus"> </div>
            </div>
        </div>
        @component('input_image', ['width' => 1400, 'height' => 300, 'label' => 'Image', 'src' => route('image_show', $homesection2->image)])
            image
        @endcomponent
