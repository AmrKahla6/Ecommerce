...
$termResourceKey = "9197661409719197";
print_r($_REQUEST);
exit();
...
if ($ResErrorText == null && $ResErrorNo == null) {
    ...
    if ($ResTranData != null) {
        ...
        header("Location: http://juicey.test:8080/result.php?" . $decrytedData);
        exit();
    }
} else {
    header("Location: http://juicey.test:8080/result.php?" . "Error=" . $ResErrorNo . "&ErrorText=" . $ResErrorText . "&trackid=" . $ResTrackID . "&amt=" . $ResAmount . "&paymentid=" . $ResPaymentId);
    exit();
}
